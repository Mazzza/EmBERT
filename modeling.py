...
#         self.value_task_emb = nn.Linear(config.num_tasks, self.self.all_head_size)
        ...
        self.query_task_emb = nn.Embedding(config.num_tasks, self.self.all_head_size)
        self.key_task_emb = nn.Embedding(config.num_tasks, self.self.all_head_size)
        self.value_task_emb = nn.Embedding(config.num_tasks, self.self.all_head_size)
    ...
    def forward(self, hidden_states, attention_mask, i):
        query_emb = self.query_task_emb(torch.tensor(i)).view((1,1,1,-1))
        key_emb = self.key_task_emb(torch.tensor(i)).view((1,1,1,-1))
        value_emb = self.value_task_emb(torch.tensor(i)).view((1,1,1,-1))
...
class BERTAttention(nn.Module):
    def __init__(self, config, multi_params=None, houlsby=False):
        ...
        if config.our_attn:
            self.self = BERTLukaszSelfAttention(config, multi_params)
        else:
